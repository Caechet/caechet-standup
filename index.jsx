...
function renderLine(line, i) {
  ...
  if (sections.some(s => line.toUpperCase().startsWith(s))) {
  ...
  if (line.includes('[CRITICAL]') || line.includes('CRITICAL]')) {
    return <div key={i} style={{ display: 'flex', gap: 10, padding: '10px 14px', background: 'rgba(224,51,51,0.06)', borderLeft: '3px solid #E03333', borderRadius: '0 8px 8px 0', marginBottom: 6 }}><p style={{ ...s, fontSize: 13, color: B.carbon, lineHeight: 1.55 }}>{line}</p></div>;
  }
  if (line.includes('[HIGH]')) {
    return <div key={i} style={{ display: 'flex', gap: 10, padding: '10px 14px', background: 'rgba(217,119,6,0.06)', borderLeft: '3px solid #D97706', borderRadius: '0 8px 8px 0', marginBottom: 6 }}><p style={{ ...s, fontSize: 13, color: B.carbon, lineHeight: 1.55 }}>{line}</p></div>;
  }
  if (line.includes('[WATCH]')) {
    return <div key={i} style={{ display: 'flex', gap: 10, padding: '10px 14px', background: cobaltA(0.04), borderLeft: `3px solid ${B.cobalt}`, borderRadius: '0 8px 8px 0', marginBottom: 6 }}><p style={{ ...s, fontSize: 13, color: B.carbon, lineHeight: 1.55 }}>{line}</p></div>;
  }
  ...
  if (line.match(/^[1-9]\./)) {
    const num = line.match(/^([1-9]\.)/)[1].replace('.', '');
          ...
          <div style={{ maxWidth: 860, margin: '0 auto', padding: '24px 40px 20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <div>
                  <div style={{ fontFamily: "'Archivo', system-ui", fontWeight: 700, fontSize: 28, letterSpacing: '-0.035em', color: B.banana, lineHeight: 1 }}>
                    cæchet<span style={{ fontSize: 6, marginLeft: '0.05em', verticalAlign: 'super', fontWeight: 700 }}>®</span>
                  </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                {data?.date && <span style={{ fontFamily: 'Lato', fontSize: 10, color: bananaA(0.5), letterSpacing: '0.1em' }}>{refreshing ? 'REFRESHING...' : `UPDATED ${formatRelativeTime(data.date)}`}</span>}
                <button onClick={() => fetchStandup(true)} disabled={refreshing} style={{ fontFamily: 'Lato', fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', padding: '8px 16px', background: 'transparent', color: B.banana, border: `1px solid ${bananaA(0.35)}`, borderRadius: 6, cursor: refreshing ? 'default' : 'pointer', opacity: refreshing ? 0.5 : 1 }}>
                  ↺ REFRESH
                </button>
              </div>
            </div>
        ...
        <div style={{ maxWidth: 860, margin: '0 auto', padding: '32px 40px 80px' }}>
          ...
          {!loading && error && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '80px 20px', gap: 16 }}>
              ...
              <p style={{ fontFamily: 'Lato', fontSize: 13, color: cobaltA(0.35), textAlign: 'center', maxWidth: 360, margin: 0, lineHeight: 1.6 }}>Make runs daily at 9:00 AM ET. Come back after your standup has been generated.</p>
            </div>
          )}
